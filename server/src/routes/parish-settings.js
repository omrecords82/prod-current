...
// NOTE: registered before /:category so "record-columns" is never parsed as a category.
...
router.get('/:churchId/record-columns/:recordType', async (req, res) => {
  try {
    ...
    if (!RECORD_TABLE_BY_TYPE[recordType]) {
      return res.status(400).json({
        error: `Invalid recordType. Must be one of: ${Object.keys(RECORD_TABLE_BY_TYPE).join(', ')}`,
      });
    }

    const tableName = await recordTableConfig.resolveRecordTableName(churchId, recordType);
    if (!tableName) {
      return res.status(404).json({
        error: `No ${recordType} record table found for church ${churchId}`,
        fallback: true,
      });
    }
    ...
    res.json({ church_id: churchId, recordType, table: tableName, columns });
  } catch (err) {
