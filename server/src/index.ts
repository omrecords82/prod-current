...
// Legacy prompt/platform workflow APIs quarantined 2026-06-08 (see step-2 workflow cleanup).
const retiredWorkflowApisRouter = express.Router();
const workflowApiGone = (req: any, res: any) => {
...
retiredWorkflowApisRouter.all('*', workflowApiGone);
...
app.use('/api/workflows', retiredWorkflowApisRouter);
app.use('/api/workflow-templates', retiredWorkflowApisRouter);
app.use('/api/learning', retiredWorkflowApisRouter);
console.log('⛔ [Server] Legacy /api/workflows, /api/workflow-templates, /api/learning → 410 Gone');
