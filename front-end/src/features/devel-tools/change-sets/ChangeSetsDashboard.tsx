...
const ChangeSetsDashboard: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewStartDate('');
      setNewEndDate('');
      navigate(`/om-daily/change-sets/${res.data.change_set.id}`);
    } catch (err: any) {
        ...
        {show.map(cs => {
          ...
          return (
            <Tooltip key={cs.id} title={`${cs.code} — ${cs.title}`} placement="top">
              <Box
                onClick={(e) => { e.stopPropagation(); navigate(`/om-daily/change-sets/${cs.id}`); }}
                sx={{
                  px: 0.5,
                  py: 0.2,
                  borderRadius: 0.5,
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            ...
            <Button
              ...
              onClick={() => navigate('/om-daily/sdlc-wizard')}
            >
              SDLC Wizard
            </Button>
            <Button
              ...
              onClick={() => navigate('/om-daily/change-sets/releases')}
            >
              Release History
            </Button>
            <Button
                  ...
                  changeSets.map((cs) => {
                    ...
                    return (
                      <TableRow
                        ...
                        onClick={() => navigate(`/om-daily/change-sets/${cs.id}`)}
                      >
                        <TableCell>
